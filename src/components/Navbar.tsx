...
export default function Navbar() {
  ...
  return (
    ...
    >
      ...
      {/* Mobile menu - only in DOM when open so it's closed on load and opens only on click */}
      {menuOpen && (
        <div
          id={MOBILE_MENU_ID}
          className="md:hidden bg-card border-b border-border px-4 sm:px-6 py-5 flex flex-col gap-3 max-h-[calc(100vh-4rem)] overflow-y-auto"
          aria-hidden={false}
        >
          {navLinks.map((link) => (
            <a
